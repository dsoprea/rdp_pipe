...
import rdp_client.entrypoint.rdp_remote


def test_default_sock_filepath():
    """Default socket path is /tmp/rdp.sock when omitted."""

    parser = rdp_client.entrypoint.rdp_remote.build_argument_parser()
    arguments = parser.parse_args(["command_receive_geometry"])

    assert arguments.sock_filepath == rdp_client.command_socket.DEFAULT_COMMAND_SOCKET_PATH
...
def test_help_flag_exits_successfully():
    ...
    with pytest.raises(SystemExit) as exit_info:
        rdp_client.entrypoint.rdp_remote.main(["-h"])
...
def test_send_key_requires_keys_or_key():
    ...
    with pytest.raises(SystemExit):
        rdp_client.entrypoint.rdp_remote.main(["command_send_key"])
...
def test_send_key_rejects_both_keys_and_key():
    ...
    with pytest.raises(SystemExit):
        rdp_client.entrypoint.rdp_remote.main(
...
def test_send_geometry_rejects_partial_dimensions():
    ...
    with pytest.raises(SystemExit):
        rdp_client.entrypoint.rdp_remote.main(["command_send_geometry", "1920"])
...
def test_build_request_body_receive_geometry():
    ...
    request_body = rdp_client.entrypoint.rdp_remote.build_request_body_for_arguments(
        arguments)
...
def test_supported_commands_have_subparsers():
    ...
    assert set(rdp_client.entrypoint.rdp_remote.SUBPARSER_REGISTRARS_BY_WIRE_COMMAND.keys()) \
        == set(rdp_client.command_socket.SUPPORTED_COMMANDS)
