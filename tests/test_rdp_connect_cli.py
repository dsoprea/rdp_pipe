"""CLI validation tests for rdp_connect."""

import pytest

import rdp_client.entrypoint.rdp_connect
...
def test_headless_requires_command_socket():
    ...
    with pytest.raises(SystemExit):
        rdp_client.entrypoint.rdp_connect.main(
            ["--headless", "10.0.0.5"])
...
def test_command_socket_without_headless_parses():
    ...
    parser = rdp_client.entrypoint.rdp_connect.build_argument_parser()
...
def test_help_flag_exits_successfully():
    ...
    with pytest.raises(SystemExit) as exit_info:
        rdp_client.entrypoint.rdp_connect.main(["-h"])
