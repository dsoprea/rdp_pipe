...
def test_ordered_connection_steps_include_every_defined_label():
    ...
    for step_identifier in rdp_pipe.connection_progress.CONNECTION_STEP_LABELS.keys():
        assert step_identifier in rdp_pipe.connection_progress.ORDERED_CONNECTION_STEPS


def test_get_connection_step_label_returns_operator_text():
    """Known steps return stable operator-facing labels."""

    label = rdp_pipe.connection_progress.get_connection_step_label(
...
def test_get_connection_step_index_orders_preparing_before_ready():
    ...
    preparing_index = rdp_pipe.connection_progress.get_connection_step_index(
        rdp_pipe.connection_progress.CONNECTION_STEP_PREPARING)

    ready_index = rdp_pipe.connection_progress.get_connection_step_index(
...
def test_unknown_step_label_raises_value_error():
    ...
    with pytest.raises(ValueError):
        rdp_pipe.connection_progress.get_connection_step_label("not-a-step")
...
def test_unknown_step_index_raises_value_error():
    ...
    with pytest.raises(ValueError):
        rdp_pipe.connection_progress.get_connection_step_index("not-a-step")
