...
def test_encode_monitor_layout_pdu_header_and_length():
    ...
    pdu_bytes = rdp_pipe.display_control.encode_monitor_layout_pdu(1280, 800)

    message_type, pdu_length, monitor_layout_size, monitor_count = struct.unpack(
        "<IIII",
        pdu_bytes[0:16])

    assert message_type == rdp_pipe.display_control.PDU_TYPE_MONITOR_LAYOUT
    assert pdu_length == 16 + rdp_pipe.display_control.MONITOR_LAYOUT_STRUCT_SIZE
    assert monitor_layout_size == rdp_pipe.display_control.MONITOR_LAYOUT_STRUCT_SIZE
    assert monitor_count == 1
    assert len(pdu_bytes) == 16 + rdp_pipe.display_control.MONITOR_LAYOUT_STRUCT_SIZE


def test_encode_monitor_layout_pdu_even_width():
    """Width is forced even per MS-RDPEDISP."""

    pdu_bytes = rdp_pipe.display_control.encode_monitor_layout_pdu(1281, 800)
    layout_bytes = pdu_bytes[16:]

    flags, left, top, width, height = struct.unpack("<iiiii", layout_bytes[0:20])

    assert flags == rdp_pipe.display_control.MONITOR_PRIMARY_FLAG
    assert left == 0
    assert top == 0
    assert width == 1280
    assert height == 800
...
def test_decode_caps_pdu_round_trip_fields():
    ...
    caps = rdp_pipe.display_control.decode_caps_pdu(caps_bytes)
...
def test_record_sent_resolution_skips_initial_duplicate_layout_pdu():
    ...
    async def run_recorded_resolution_requests():
        channel = rdp_pipe.display_control.DisplayControlChannel()
        channel._caps = rdp_pipe.display_control.DisplayControlCaps(1, 8192, 8192)
        transmit_count = 0
...
def test_clamp_even_display_width_bounds():
    ...
    assert rdp_pipe.display_control.clamp_even_display_width(100) == 200
    assert rdp_pipe.display_control.clamp_even_display_width(9000) == 8192
    assert rdp_pipe.display_control.clamp_even_display_width(801) == 800
